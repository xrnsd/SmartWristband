...
public class PermissionUtil {
    ...
    public static List<String> findDeniedPermissions(String... permission){
        ...
        for(String value : permission){
            if(ContextCompat.checkSelfPermission( BaseAppcation.getInstance(),value) != PackageManager.PERMISSION_GRANTED){
    ...
    public static boolean isGrantedDrawOverlays() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            AppOpsManager aom = (AppOpsManager) BaseAppcation.getInstance().getSystemService(Context.APP_OPS_SERVICE);
            if (aom == null) return false;
            int mode = aom.checkOpNoThrow(
                    "android:system_alert_window",
                    android.os.Process.myUid(),
                    BaseAppcation.getInstance().getPackageName()
            );
            return mode == AppOpsManager.MODE_ALLOWED || mode == AppOpsManager.MODE_IGNORED;
        }
        return Settings.canDrawOverlays(BaseAppcation.getInstance());
    }
    /**
    ...
    private static void startWriteSettingsActivity(final Activity activity, final int requestCode) {
        ...
        intent.setData(Uri.parse("package:" + BaseAppcation.getInstance().getPackageName()));
    ...
    public static void launchAppDetailsSettings() {
        ...
        BaseAppcation.getInstance().startActivity(intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK));
    }
    private static boolean isIntentAvailable(final Intent intent) {
        return BaseAppcation.getInstance()
                .getPackageManager()
