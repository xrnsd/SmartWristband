 ...
 * 轨迹纠偏功能 示例 使用起来更简单
 */
public class TraceActivityUpload extends Activity {
    private List<TraceLocation> mListPoint = new ArrayList<>();
    private List<TraceLocation> originPosList;
    private LBSTraceClient lbsTraceClient;

    private static final String TAG = "TraceActivityUpload_123456";
    ...
    private void init() {
        if (aMap == null) {
            ...
            mPreferLocationList = MMKV.mmkvWithID("WristbandInfoLocationList");
        }
        initView();
        ...
        getWindow().getDecorView().post(new Runnable() {
            @Override
            public void run() {
                MMKV.Editor editor = mPreferLocationList.edit();
                int count = mPreferLocationList.getInt("WristbandInfoListLengthAll", 0);
                if (count <= 0) {
                    Toast.makeText(getApplicationContext(), "轨迹数据为空", Toast.LENGTH_LONG).show();
                    return;
                }
                ...
                for (int index = 0; index < count; index++) {
                    Latitude = Double.parseDouble(mPreferLocationList.getString("WristbandInfo.Latitude" + index, "0"));
                    Longitude = Double.parseDouble(mPreferLocationList.getString("WristbandInfo.Longitude" + index, "0"));
...
//        }
    ...
    private TrajectoryFilter mSpeedFilter;
